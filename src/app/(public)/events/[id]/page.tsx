...
export async function generateMetadata({
  ...
  if (!event && id !== "1") return { title: "Event Not Found" };

  const title = event?.title || "Global Leadership Summit 2024";
  const description =
    event?.description ||
    "The 2024 Global Leadership Summit brings together the world's most influential decision-makers.";
  const imageUrl =
    event?.imageUrl ||
    ...
    ? {
        ...
        lineup: (event.lineup?.length
            ...
            }))) ?? [
          {
            ...
            org: "Vanguard Global",
            imageUrl:
              "https://lh3.googleusercontent.com/aida-public/AB6AXuCHeDbTvuBRTti5V7FGndJFympx3Jxi-WkFGpHdPvOWDsar4SyKuxCwXgJnXvBAt22vVJM1FtfOct6Rk7RVXwjQBCc9hDUnWxPSmXAZojfGm5m9g1mEo7ki6lcd8nM-SSpguzPKw68UgxQUFn3DxlwmDS7IIgdWTm78X7Pyj76SZeyZfjLvkE6pCr76vf2kDJ6xBmznaJfhhFbregPaU8ici3lwrAwRJzcv6w_yYoUccFLh9RyNuZSCyk15wBrMvDHyU6tmGytOBzkj",
          },
          {
            name: "Helena Rosseau",
            role: "Headlining Artist",
            org: "Blue Chip Productions",
            imageUrl:
              "https://lh3.googleusercontent.com/aida-public/AB6AXuCqy4YCPov8kDkWqObPV-0R8kcMGFIc66I8DttSxvnxgTEL1o-3WYvd3P7OpjAeLcqVyNogfyCJIOZB1KBT5m5qVinHGqfRkuBJGbh-aADb0LDpyqWt0IpyFrWJfdO-xzjpNICupjb_2aWZXDuppDuBZ1n1i1OH04hWyLYLYxh6CeVMsiFcrdUwp33E43VKGO81aeuiXfaFHoeGkmcX4DSiOgBQcP_wnZTNDcDK7w0G62mMNIiJTW6Su8siikZntsbDs1dG49DElFxF",
          },
          {
            name: "Marcus Thorne",
            role: "Workshop Host",
            org: "The Sovereign Fund",
            imageUrl:
              "https://lh3.googleusercontent.com/aida-public/AB6AXuDAumuLjWa3WmT376J-KyBuxJ3mjEyRJ8ldl_AQOT7LaGXQe1tSfekP7EI3bGAvcNIXgib7RItNJhwplJm0oMFseheiyWziUTD4V8YyLZuM-hpxdtQYdHOVwYLIKkeaF9BFQigeWvK752CYFyXDsMLHn2rY7mh9aP53L1Qnec8yhogCd33fvMDEcz-9Rvj6hPtjbDDeed2A0gIlcktfWH6h74rWVHeRt2ID2kYZQ6aD5Ym1zCft5wYFgMjlujhQbo34kFdvPu21R3XT",
          },
        ],
          ...
          : [
              {
                ...
                description: "Strategic leaders shaping the future vision.",
                icon: "corporate_fare",
              },
              {
                name: "Asset Managers",
                description: "Professionals overseeing high-stakes capital.",
                icon: "account_balance",
              },
              {
                name: "Policy Architects",
                description: "Individuals crafting digital age frameworks.",
                icon: "insights",
              },
              {
                name: "Strategy Partners",
                description: "Consultants driving operational excellence.",
                icon: "group_work",
              },
            ],
      }
        ...
        <div className="flex-1 space-y-16">
          ...
          <section className="bg-surface-container-low/50 p-10 rounded-2xl border border-outline-variant/10 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.attendeeTypes.map(
                (type: { name: string; description: string; icon: string }, i: number) => (
                  <div
                    key={i}
                    className="bg-surface-container-lowest p-6 rounded-xl flex items-start gap-4 border border-outline-variant/10"
                  >
          ...
          <section className="space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {data.lineup.map(
                (
                  member: { name: string; role: string; org?: string; imageUrl?: string },
                  i: number
                ) => (
                  <div key={i} className="group">
                    ...
                    {member.org && (
                      <p className="text-on-surface-variant text-sm font-body">{member.org}</p>
                    )}
                  </div>
                )
              )}
