...
export function StaffRosterClient({ roster }: StaffRosterClientProps) {
  const [searchTerm, setSearchTerm] = useState("");

  const filteredRoster = roster.filter((staff: any) => 
    staff.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    staff.staffProfile?.skills?.some((skill: string) => 
      skill.toLowerCase().includes(searchTerm.toLowerCase())
    )
  );

  const handleExport = () => {
    const headers = ["Name", "Email", "Skills", "VerifiedOn", "EarningsBalance"];
    const data = filteredRoster.map((staff: any) => ({
      Name: staff.name,
      Email: staff.email,
      Skills: staff.staffProfile?.skills?.join("; ") || "",
      VerifiedOn: new Date(staff.updatedAt).toLocaleDateString(),
      EarningsBalance: staff.staffProfile?.earnings?.balance || 0,
    }));
    downloadCSV("master-roster.csv", headers, data);
  };
    ...
    <>
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <p className="mt-2 text-on-surface-variant font-medium font-body">{filteredRoster.length} Verified service providers in view</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-6 py-3 bg-surface-container-high hover:bg-surface-container-highest rounded-xl text-xs font-bold uppercase tracking-widest text-on-surface-variant transition-all active:scale-95"
          >
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-outline-variant/10 overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-surface-container-low border-b border-outline-variant/5">
              ...
              <th className="px-10 py-6 text-[10px] font-black uppercase tracking-widest text-on-surface-variant text-right">Action</th>
            </tr>
          </thead>
        ...
        {filteredRoster.length === 0 && (
          <div className="px-10 py-20 text-center">
            <p className="text-on-surface-variant font-medium">No verified staff found matching your criteria.</p>
          </div>
        )}
      </div>
    </>
